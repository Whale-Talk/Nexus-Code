...
### 修复
- **状态栏样式对齐**：默认 statusline 升级为绿色模型名+上下文窗口 · 青色当前用量 · 橙色 5h/7d 限额
...
### 修复
- **Grep 工具 ripgrep 二进制缺失**：builtin 路径解析错误（src/utils/vendor 而非项目根）。
  现在探测 vendor/ripgrep（自部署）→ node_modules SDK 自带 vendor（依赖回退）
...
### 修复
- **Windows 跨目录遮蔽（spawn EINVAL）**（v1.0.7 残留 bug 报告）：
  - 查找循环反转为「候选外层 × 目录内层」：任何目录的 `bun.exe` 优先于
    所有 `bun.cmd`，消除 PATH 靠前目录的 cmd shim 对后置目录真实二进制的遮蔽
  - `.cmd` 仅在完全没有 `bun.exe` 时命中，spawn 此时走 `shell: true`
    （cmd.exe 包装）兜底，不再抛 EINVAL
...
### 修复
- **Windows bun 候选顺序**（v1.0.6 安装反馈）：win32 候选名改为 `bun.exe → bun.cmd → 裸名`。
  原实现裸名排第一，npm 全局安装的扩展名缺失 shim 被 existsSync 命中、spawn ENOENT。
  现在可执行扩展名优先，真实二进制不会被 shim 遮蔽
- **ENOENT 错误信息区分**：区分"定位到 shim 但无法执行"（给出 PATH/BUN_BINARY 修复指引）
  与"bun 未找到"（给出重装指引），不再误导用户重装
...
| 1.0.10 | 2026-08-14 | Grep 工具 ripgrep 路径修复 |
| 1.0.11 | 2026-08-14 | 状态栏样式对齐 |
