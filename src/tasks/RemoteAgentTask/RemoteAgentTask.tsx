...
export function formatPreconditionError(error: BackgroundRemoteSessionPrecondition): string {
  switch (error.type) {
    ...
    case 'no_remote_environment':
      return 'No cloud environment available. Set one up at https://github.com/Whale-Talk/Nexus-Code';
    ...
    case 'github_app_not_installed':
      return 'The Nexus GitHub app must be installed on this repository first.\nhttps://github.com/apps/claude/installations/new';
    case 'policy_blocked':
 ...
 *   {type:'system', subtype:'hook_progress', stdout:'...'}. Nexus never
...
function extractTodoListFromLog(log: SDKMessage[]): TodoList {
  ...
  const input = todoListMessage.message.content.find((block): block is ToolUseContentBlock => block.type === 'tool_use' && block.name === TodoWriteTool.name)?.input;
  if (!input) {
    return [];
  }
...
function startRemoteSessionPolling(taskId: string, context: TaskContext): () => void {
  ...
  const poll = async (): Promise<void> => {
    ...
    try {
      ...
      // stableIdle is a prompt-mode completion signal (Nexus stops writing
