...
export const DESCRIPTION = 'Get or set Nexus Code configuration settings.'
...
export function generatePrompt(): string {
  ...
  return `Get or set Nexus Code configuration settings.

  View or change Nexus Code settings. Use when the user requests configuration changes, asks about current settings, or when adjusting a setting would benefit them.
...
- Change model: { "setting": "model", "value": "quark" }
...
${lines.join('\n')}`
    ...
    return `## Model
- model - Override the default model (atom, quark, electron, or a full model ID)`
  }
}
