  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (isPowerShellParameter(arg, argElementType)) {
      ...
      if (matchesParam(paramLower, config.pathParams)) {
        ...
        // SECURITY: A Variable elementType (e.g., $env:NEXUS_API_KEY) in any
...
function checkPathConstraintsForStatement(
  ...
  // Runtime writes /project/.claude/settings.json (Nexus's permission config).
  //
      ...
      if (!allowed) {
        ...
        const message =
          ...
          decisionReason?.type === 'safetyCheck'
            ...
            : `${canonical} targeting '${resolvedPath}' was blocked. For security, Nexus Code may only access files in the allowed working directories for this session: ${dirListStr}.`
