...
export async function powershellToolHasPermission(
  ...
  // `Write-Output $env:NEXUS_API_KEY`, pushes allow to decisions[], and
  ...
  for (const { text: subCmd, element, statement } of subCommands) {
    ...
    if (
      ...
      // `Write-Output $env:NEXUS_API_KEY`. Apply the same argLeaksValue
    ...
    // .claude/settings.json (a Nexus config file the path validator didn't check).
