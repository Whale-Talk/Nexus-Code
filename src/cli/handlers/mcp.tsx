...
export async function mcpAddFromDesktopHandler(options: {
  ...
  try {
    ...
    if (Object.keys(servers).length === 0) {
      cliOk('No MCP servers found in Nexus Desktop configuration or configuration file does not exist.');
    }
    const {
      unmount
...
export async function mcpResetChoicesHandler(): Promise<void> {
  ...
  cliOk('All project-scoped (.mcp.json) server approvals and rejections have been reset.\n' + 'You will be prompted for approval next time you start Nexus Code.');
}
