...
\`\`\`json
{
  "model": "atom",  // or "quark", "electron", full model ID
  "agent": "agent-name",
...
Hooks run commands at specific points in Nexus Code's lifecycle.
...
| Stop | - | Run when Nexus stops (including clear, resume, compact) |
...
Modify Nexus Code configuration by updating settings.json files.
...
User: "Format my code after Nexus writes it"
...
export function registerUpdateConfigSkill(): void {
  registerBundledSkill({
    name: 'update-config',
    description:
      'Use this skill to configure the Nexus Code harness via settings.json. Automated behaviors ("from now on when X", "each time X", "whenever X", "before/after X") require hooks configured in settings.json - the harness executes these, not Nexus, so memory/preferences cannot fulfill them. Also use for: permissions ("allow X", "add permission", "move permission to"), env vars ("set X=Y"), hook troubleshooting, or any changes to settings.json/settings.local.json files. Examples: "allow npm commands", "add bq permission to global settings", "move permission to user settings", "set DEBUG=true", "when claude stops show X". For simple settings like theme/model, use Config tool.',
    allowedTools: ['Read'],
    userInvocable: true,
