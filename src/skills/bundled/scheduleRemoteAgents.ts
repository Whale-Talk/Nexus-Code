...
function formatConnectorsInfo(connectors: ConnectorInfo[]): string {
  if (connectors.length === 0) {
    return 'No connected MCP connectors found. The user may need to connect servers at https://github.com/Whale-Talk/Nexus-Code'
  }
...
You are helping the user schedule, update, list, or run **remote** Nexus Code agents. These are NOT local cron jobs — each trigger spawns a fully isolated remote session (CCR) in remote cloud infrastructure on a cron schedule. The agent runs in a sandboxed environment with its own git checkout, tools, and optional MCP connections.
...
You CANNOT delete triggers. If the user asks to delete, direct them to: https://github.com/Whale-Talk/Nexus-Code/scheduled
...
**Important:** Infer what services the agent needs from the user's description. For example, if they say "check Datadog and Slack me errors," the agent needs both Datadog and Slack connectors. Cross-reference against the list above and warn if any required service isn't connected. If a needed connector is missing, direct the user to https://github.com/Whale-Talk/Nexus-Code to connect it first.
...
5. **Validate connections** — Infer what services the agent will need from the user's description. For example, if they say "check Datadog and Slack me errors," the agent needs both Datadog and Slack MCP connectors. Cross-reference with the connectors list above. If any are missing, warn the user and link them to https://github.com/Whale-Talk/Nexus-Code to connect first.${gitRepoUrl ? ` The default git repo is already set to \`${gitRepoUrl}\`. Ask the user if this is the right repo or if they need a different one.` : ' Ask which git repos the remote agent needs cloned into its environment.'}
6. **Review and confirm** — Show the full configuration before creating. Let them adjust.
7. **Create it** \u2014 Call \`${REMOTE_TRIGGER_TOOL_NAME}\` with \`action: "create"\` and show the result. The response includes the trigger ID. Always output a link at the end: \`https://github.com/Whale-Talk/Nexus-Code/scheduled/{TRIGGER_ID}\`
...
- These are REMOTE agents — they run in remote cloud, not on the user's machine. They cannot access local files, local services, or local environment variables.
...
- To delete a trigger, direct users to https://github.com/Whale-Talk/Nexus-Code/scheduled
...
export function registerScheduleRemoteAgentsSkill(): void {
  registerBundledSkill({
    ...
    whenToUse:
      'When the user wants to schedule a recurring remote agent, set up automated tasks, create a cron job for Nexus Code, or manage their scheduled agents/triggers.',
    userInvocable: true,
    isEnabled: () =>
    ...
    async getPromptForCommand(args: string, context: ToolUseContext) {
      if (!getClaudeAIOAuthTokens()?.accessToken) {
        return [
          {
            type: 'text',
            text: 'You need to authenticate with an account first. Run /login, then try /schedule again.',
          },
        ]
      }
      ...
      if (environments.length === 0) {
        try {
          ...
          return [
            {
              type: 'text',
              text: 'No remote environments found, and we could not create one automatically. Visit https://github.com/Whale-Talk/Nexus-Code to set one up, then run /schedule again.',
            },
          ]
        }
      }
      ...
      if (connectors.length === 0) {
        setupNotes.push(
          `No MCP connectors — connect at https://github.com/Whale-Talk/Nexus-Code if needed.`,
        )
      }
