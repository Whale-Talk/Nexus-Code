...
function Install({
    ...
    async function run() {
      try {
        ...
        if (result.lockFailed) {
          throw new Error('Could not install - another process is currently installing Nexus. Please try again in a moment.');
        }
  ...
  useEffect(() => {
    if (state.type === 'success') {
      ...
      setTimeout(onDone, 2000, 'Nexus Code installation completed successfully', {
        display: 'system' as const
      });
    } else if (state.type === 'error') {
      // Give error message time to render before exiting
      setTimeout(onDone, 3000, 'Nexus Code installation failed', {
        display: 'system' as const
      });
    }
  }, [state, onDone]);
  return <Box flexDirection="column" marginTop={1}>
      ...
      {state.type === 'installing' && <Text color="claude">
          Installing Nexus Code native build {state.version}...
        </Text>}
      ...
      {state.type === 'success' && <Box flexDirection="column" gap={1}>
          <Box>
            ...
            <Text color="success" bold>
              Nexus Code successfully installed!
            </Text>
          </Box>
...
export const install = {
  ...
  name: 'install',
  description: 'Install Nexus Code native build',
