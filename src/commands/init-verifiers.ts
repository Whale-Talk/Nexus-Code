...
3. **Installed verification packages** (for web apps)
   ...
   - Check MCP configuration (.mcp.json) for browser automation tools:
     ...
     - Nexus Chrome Extension MCP (browser-use via Nexus's Chrome extension)
...
2. **If NO browser automation tools are detected**, ask if they want to install/configure one:
   ...
   - Options to offer:
     ...
     - **Nexus Chrome Extension** - Uses the Nexus Chrome extension for browser interaction (requires the extension installed in Chrome)
...
4. **If user chooses Chrome DevTools MCP or Nexus Chrome Extension**:
   ...
   - For Nexus Chrome Extension, inform them they need the extension installed from the Chrome Web Store
...
**All verifier skills are created in the project root's \`.claude/skills/\` directory.** This ensures they are automatically loaded when Nexus runs in the project.
