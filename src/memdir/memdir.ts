 ...
 *
 * Shared by buildMemoryPrompt and nexusmd getMemoryFiles (previously
 ...
 * Shipped because Nexus was burning turns on `ls`/`mkdir -p` before writing.
 ...
 * Used by agent memory (which has no getNexusMds() equivalent).
 */
 ...
 * files + MEMORY.md. MEMORY.md is still loaded into context (via nexusmd.ts)
