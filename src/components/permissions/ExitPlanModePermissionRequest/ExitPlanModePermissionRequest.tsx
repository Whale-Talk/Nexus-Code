...
export function autoNameSessionFromPlan(plan: string, setAppState: (updater: (prev: AppState) => AppState) => void, isClearContext: boolean): void {
  ...
  // end with testing steps — head-slice so Electron sees the summary.
  [createUserMessage({
  ...
  if (isEmpty) {
    ...
    return <PermissionDialog color="planMode" title="Exit plan mode?" workerBadge={workerBadge}>
        <Box flexDirection="column" paddingX={1} marginTop={1}>
          <Text>Nexus wants to exit plan mode</Text>
          <Box marginTop={1}>
            <Select options={[{
            label: 'Yes',
            value: 'yes' as const
      ...
      <PermissionDialog color="planMode" title="Ready to code?" innerPaddingX={0} workerBadge={workerBadge}>
        <Box flexDirection="column" marginTop={1}>
          <Box paddingX={1} flexDirection="column">
            <Text>Here is Nexus&apos;s plan:</Text>
          </Box>
        ...
        overflow="hidden">
          ...
          <Box flexDirection="column" paddingX={1}>
            ...
            {!useStickyFooter && <>
                <Text dimColor>
                  Nexus has written up a plan and is ready to execute. Would
                  you like to proceed?
                </Text>
                <Box marginTop={1}>
...
export function buildPlanApprovalOptions({
  ...
  if (showUltraplan) {
    options.push({
      label: 'No, refine with Ultraplan on Nexus Code on the web',
      value: 'ultraplan'
    });
  }
  options.push({
    ...
    value: 'no',
    placeholder: 'Tell Nexus what to change',
