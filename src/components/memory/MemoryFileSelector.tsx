...
import { getMemoryFiles, type MemoryFileInfo } from '../../utils/nexusmd.js';
...
export function MemoryFileSelector(t0) {
  ...
  const userMemoryPath = join(getClaudeConfigHomeDir(), "NEXUS.md");
  const projectMemoryPath = join(getOriginalCwd(), "NEXUS.md");
  ...
  const memoryOptions = allMemoryFiles.map(file => {
    ...
    if (file.type === "User" && !file.isNested) {
      description = "Saved in ~/.claude/NEXUS.md";
    } else {
      if (file.type === "Project" && !file.isNested && file.path === projectMemoryPath) {
        description = `${isGit ? "Checked in at" : "Saved in"} ./NEXUS.md`;
      } else {
        if (file.parent) {
