...
/**
 * Get Nexus Code MCP configurations (excludes claude.ai servers from the
 ...
 * @returns Nexus Code server configurations with appropriate scopes
 */
  ...
  for (const [name, config] of Object.entries(schemaResult.data.mcpServers)) {
    ...
    if (
      ...
      errors.push({
        ...
        suggestion: `Change command to "cmd" with args ["/c", "npx", ...]. See: https://github.com/Whale-Talk/Nexus-Code#configure-mcp-servers`,
        mcpErrorMetadata: {
          scope,
          serverName: name,
          severity: 'warning',
