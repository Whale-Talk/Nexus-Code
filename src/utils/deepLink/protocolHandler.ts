...
export async function handleDeepLinkUri(uri: string): Promise<number> {
  ...
  // baked at registration time, and we want the terminal-launched Nexus to
...
/**
 * Resolve the working directory for the launched Nexus instance.
 * Precedence: explicit cwd > repo lookup (MRU clone) > home.
 * A repo that isn't cloned locally is not an error — fall through to home
 * so a web link referencing a repo the user doesn't have still opens Nexus.
 *
