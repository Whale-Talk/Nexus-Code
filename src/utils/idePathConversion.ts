 ...
 * Handles conversions between Nexus's environment and the IDE's environment
 */
...
export interface IDEPathConverter {
  /**
   * Convert path from IDE format to Nexus's local format
   * Used when reading workspace folders from IDE lockfile
   */
  toLocalPath(idePath: string): string

  /**
   * Convert path from Nexus's local format to IDE format
...
/**
 * Converter for Windows IDE + WSL Nexus scenario
 */
