/**
 * Adapter layer that wraps @anthropic-ai/sandbox-runtime with Nexus CLI-specific integrations.
 * This file provides the bridge between the external sandbox-runtime package and Nexus CLI's
...
/**
 * Resolve Nexus Code-specific path patterns for sandbox-runtime.
 *
 * Nexus Code uses special path prefixes in permission rules:
...
/**
 * Convert Nexus Code settings format to SandboxRuntimeConfig format
...
export function convertToSandboxRuntimeConfig(
  ...
  // Always include current directory and Nexus temp directory as writable
  ...
  // This blocks settings in the original working directory (where Nexus Code started)
  ...
  // (auto-discovered, auto-loaded, full Nexus capabilities) so they need the
  ...
  // core.fsmonitor) escapes the sandbox when Nexus's unsandboxed git runs.
  //
 ...
 * Nexus's unsandboxed git calls can see them. See the SECURITY block above
 ...
 * This is a Nexus CLI-specific function that updates local settings.
 */
...
/**
 * Nexus CLI sandbox manager - wraps sandbox-runtime with Nexus-specific features
 */
