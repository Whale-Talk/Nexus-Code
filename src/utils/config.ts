...
export type ProjectConfig = {
  ...
  hasNexusMdExternalIncludesApproved?: boolean
  hasNexusMdExternalIncludesWarningShown?: boolean
...
const DEFAULT_PROJECT_CONFIG: ProjectConfig = {
  ...
  hasNexusMdExternalIncludesApproved: false,
  hasNexusMdExternalIncludesWarningShown: false,
}
...
export type GlobalConfig = {
  ...
  firstStartTime?: string // ISO timestamp when Nexus Code was first started on this machine

  messageIdleNotifThresholdMs: number // How long the user has to have been idle to get a notification that Nexus is done generating
  ...
  // Nexus Code usage tracking
  claudeCodeFirstTokenDate?: string // ISO timestamp of the user's first Nexus Code OAuth token
  ...
  // Effort callout tracking - shown once for Nexus Quark users
  ...
  // Nexus in Chrome settings
  hasCompletedClaudeInChromeOnboarding?: boolean // Whether Nexus in Chrome onboarding has been shown
  claudeInChromeDefaultEnabled?: boolean // Whether Nexus in Chrome is enabled by default (undefined means platform default)
  ...
  // Nexus Code hint protocol state (<claude-code-hint /> tags from CLIs/SDKs).
...
function saveConfigWithLock<A extends object>(
  ...
  try {
    ...
    if (lockTime > 100) {
      logForDebugging(
        'Lock acquisition took longer than expected - another Nexus instance may be running',
      )
...
export function getMemoryPath(memoryType: MemoryType): string {
  ...
  switch (memoryType) {
    case 'User':
      return join(getNexusConfigHomeDir(), 'NEXUS.md')
    case 'Local':
      return join(cwd, 'NEXUS.local.md')
    case 'Project':
      return join(cwd, 'NEXUS.md')
    case 'Managed':
      return join(getManagedFilePath(), 'NEXUS.md')
    case 'AutoMem':
...
export function getManagedNexusRulesDir(): string {
  return join(getManagedFilePath(), '.nexus', 'rules')
}

export function getUserNexusRulesDir(): string {
