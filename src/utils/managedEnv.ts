 ...
 * traffic (e.g., NEXUS_BASE_URL) to an attacker-controlled server.
 */
 ...
 * NEXUS_BASE_URL take effect during first-run/onboarding.
 *
 * For trusted sources (user settings, managed settings, CLI flags), ALL env vars
 * are applied — including ones like NEXUS_BASE_URL that would be dangerous
...
export function applySafeConfigEnvironmentVariables(): void {
  ...
  // NEXUS_BASE_URL — both settable via settings.env.
