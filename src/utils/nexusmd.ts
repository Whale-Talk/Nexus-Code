...
import {
  getAdditionalDirectoriesForNexusMd,
  getOriginalCwd,
...
/**
 * Checks whether a NEXUS.md file path is excluded by the nexusMdExcludes setting.
 ...
 */
function isNexusMdExcluded(filePath: string, type: MemoryType): boolean {
  ...
  // the user writes "/tmp/project/NEXUS.md" in their exclude, but the system
...
function resolveExcludePatterns(patterns: string[]): string[] {
  ...
  for (const normalized of expanded) {
    ...
    try {
      // sync IO: called from sync context (isNexusMdExcluded -> processMemoryFile -> getMemoryFiles)
...
export async function processMemoryFile(
  ...
  // Skip if path is excluded by nexusMdExcludes setting
  if (isNexusMdExcluded(filePath, type)) {
    return []
  }
...
export const getMemoryFiles = memoize(
  async (forceIncludeExternal: boolean = false): Promise<MemoryFileInfo[]> => {
    ...
    const includeExternal =
      ...
      config.hasNexusMdExternalIncludesApproved ||
      false
    ...
    // Process NEXUS.md from additional directories (--add-dir) if env var is enabled
    ...
    // memory system, not "instructions" in the NEXUS.md/rules sense.
    // Gated on !forceIncludeExternal: the forceIncludeExternal=true variant
    // is only used by getExternalNexusMdIncludes() for approval checks, not
 ...
 * Loads NEXUS.md, unconditional rules, and conditional rules for that directory.
 *
...
export type ExternalNexusMdInclude = {
  path: string
  parent: string
}

export function getExternalNexusMdIncludes(
  files: MemoryFileInfo[],
): ExternalNexusMdInclude[] {
  const externals: ExternalNexusMdInclude[] = []
...
export function hasExternalNexusMdIncludes(files: MemoryFileInfo[]): boolean {
  return getExternalNexusMdIncludes(files).length > 0
}

export async function shouldShowNexusMdExternalIncludesWarning(): Promise<boolean> {
  const config = getCurrentProjectConfig()
  if (
    config.hasNexusMdExternalIncludesApproved ||
    config.hasNexusMdExternalIncludesWarningShown
  ) {
    return false
  }

  return hasExternalNexusMdIncludes(await getMemoryFiles(true))
}
...
export function isMemoryFilePath(filePath: string): boolean {
  ...
  // NEXUS.md or NEXUS.local.md (or legacy CLAUDE.md / CLAUDE.local.md) anywhere
  if (name === 'NEXUS.md' || name === 'NEXUS.local.md' ||
