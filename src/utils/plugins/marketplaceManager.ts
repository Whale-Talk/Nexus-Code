/**
 * Marketplace manager for Nexus Code plugins
 *
...
export async function loadKnownMarketplacesConfigSafe(): Promise<KnownMarketplacesConfig> {
  try {
    ...
    // corrupted user config isn't a Nexus Code bug, shouldn't hit the error file.
    return {}
  }
}
...
async function cacheMarketplaceFromUrl(
  ...
  const headers = {
    ...
    'User-Agent': 'Nexus-Code-Plugin-Manager',
  }
    ...
    if (
      ...
      throw new Error(
        `Marketplace "${name}" has a relative source path (${entry.source.path}) ` +
          ...
          `Nexus Code version. Run 'claude marketplace remove ${name}' and ` +
