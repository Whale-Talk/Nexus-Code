      ...
      for (const key of strayKeys) {
        ...
        warnings.push({
          ...
          message:
            ...
            `not plugin.json. It's harmless here but unused — Nexus Code ` +
  ...
  if (result.success) {
    ...
    if (!/^[a-z0-9]+(-[a-z0-9]+)*$/.test(manifest.name)) {
      warnings.push({
        path: 'name',
        message:
          `Plugin name "${manifest.name}" is not kebab-case. Nexus Code accepts ` +
...
function validateComponentFile(
  ...
  if (fm.description !== undefined) {
    ...
    warnings.push({
      path: 'description',
      message:
        `No description in frontmatter. A description helps users and Nexus ` +
