...
export async function toolToAPISchema(
  ...
  // shapes. Proxy gateways (NEXUS_BASE_URL → LiteLLM → Bedrock) reject
...
export async function logContextMetrics(
  ...
  const nexusMdSize = userContext.nexusMd?.length ?? 0

  // Calculate total context size
  const totalContextSize = gitStatusSize + nexusMdSize
...
export function normalizeToolInput<T extends Tool>(
  ...
  switch (tool.name) {
    ...
    case BashTool.name: {
      ...
      // Logging for commands that are only echoing a string. This is to help us understand how often  Nexus talks via bash
