 ...
 * so NEXUS_BASE_URL/proxy/mTLS in settings would be invisible and preconnect
...
export function preconnectAnthropicApi(): void {
  ...
  // NEXUS_BASE_URL env + USE_STAGING_OAUTH + USE_LOCAL_OAUTH in one lookup.
  // NODE_EXTRA_CA_CERTS no longer a skip — init.ts applied it before this fires.
  const baseUrl =
    process.env.NEXUS_BASE_URL || getOauthConfig().BASE_API_URL
