...
export function createCronScheduler(
  ...
  async function load(initial: boolean) {
    ...
    // and fires immediately). This avoids a misleading "missed while Nexus
  ...
  function check() {
    ...
    // exists to stop two Nexus sessions in the same cwd from double-firing
...
export function buildMissedTaskNotification(missed: CronTask[]): string {
  ...
  const header =
    `The following one-shot scheduled task${plural ? 's were' : ' was'} missed while Nexus was not running. ` +
