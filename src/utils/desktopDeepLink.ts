...
/**
 * Builds a deep link URL for Nexus Desktop to resume a CLI session.
...
/**
 * Check if Nexus Desktop app is installed.
 * On macOS, checks for /Applications/Nexus.app.
...
async function isDesktopInstalled(): Promise<boolean> {
  ...
  if (platform === 'darwin') {
    // Check for Nexus.app in /Applications
    return pathExists('/Applications/Nexus.app')
...
/**
 * Detect the installed Nexus Desktop version.
...
async function getDesktopVersion(): Promise<string | null> {
  ...
  if (platform === 'darwin') {
    const { code, stdout } = await execFileNoThrow('defaults', [
      'read',
      '/Applications/Nexus.app/Contents/Info.plist',
    ...
    const installDir = join(localAppData, 'NexusCode')
    try {
...
/**
 * Build and open a deep link to resume the current session in Nexus Desktop.
...
export async function openCurrentSessionInDesktop(): Promise<{
  ...
  if (!installed) {
    return {
      success: false,
      error:
        'Nexus Desktop is not installed. Install it from https://github.com/Whale-Talk/Nexus-Code',
    }
  }
  ...
  if (!opened) {
    return {
      success: false,
      error: 'Failed to open Nexus Desktop. Please try opening it manually.',
      deepLinkUrl,
    }
  }
