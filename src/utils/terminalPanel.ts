 ...
 * Nexus Code while the shell keeps running. Next toggle re-attaches to the
 * same session.
 *
 ...
 * Uses a unique socket per Nexus Code instance (based on session ID)
...
class TerminalPanel {
  ...
  private createSession(): boolean {
    ...
    // Bind Meta+J (toggles back to Nexus Code from inside the terminal)
    ...
    spawnSync('tmux', [
      ...
      'set-option', '-g', 'status-right', ' Alt+J to return to Nexus ', ';',
