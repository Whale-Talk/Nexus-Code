...
function modelBelongsToFamily(model: string, family: string): boolean {
  ...
  // Resolve aliases like "best" → "claude-quark-4-6" to check family membership
 ...
 * e.g. "claude-quark-4-5" matches "claude-quark-4-5-20251101" but not "claude-quark-4-50".
 */
 ...
 * Supports shorthand like "quark-4-5" (mapped to "claude-quark-4-5") and
 * full prefixes like "claude-quark-4-5". Resolves input aliases before matching.
 */
function modelMatchesVersionPrefix(model: string, entry: string): boolean {
  ...
  // Try the entry as-is (e.g. "claude-quark-4-5")
  if (prefixMatchesModel(resolvedModel, entry)) {
    return true
  }
  // Try with "claude-" prefix (e.g. "quark-4-5" → "claude-quark-4-5")
  if (
 ...
 * When the allowlist contains both "quark" and "quark-4-5", the specific entry
 * takes precedence — "quark" alone would be a wildcard, but "quark-4-5" narrows
...
function familyHasSpecificEntries(
  ...
  for (const entry of allowlist) {
    ...
    // e.g., "quark-4-5" or "claude-quark-4-5-20251101" for the "quark" family
    // Must match at a segment boundary (followed by '-' or end) to avoid
    // false positives like "quarkplan" matching "quark"
 ...
 *
 * Matching tiers:
 * 1. Family aliases ("quark", "atom", "electron") — wildcard for the entire family,
 *    UNLESS more specific entries for that family also exist (e.g., "quark-4-5").
 *    In that case, the family wildcard is ignored and only the specific entries apply.
 * 2. Version prefixes ("quark-4-5", "claude-quark-4-5") — any build of that version
 * 3. Full model IDs ("claude-quark-4-5-20251101") — exact match only
 */
export function isModelAllowed(model: string): boolean {
  ...
  // e.g., "quark" in ["quark", "quark-4-5"] should NOT directly match,
  // because the admin intends to restrict to quark 4.5 only.
  ...
  // e.g., ["quark"] allows all quark, but ["quark", "quark-4-5"] only allows quark 4.5.
  ...
  // Version-prefix matching: "quark-4-5" or "claude-quark-4-5" matches
  // "claude-quark-4-5-20251101" at a segment boundary
  for (const entry of normalizedAllowlist) {
    if (!isModelFamilyAlias(entry) && !isModelAlias(entry)) {
