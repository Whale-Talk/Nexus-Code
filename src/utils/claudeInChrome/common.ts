      ...
      case 'macos': {
        ...
        try {
          ...
          if (stats.isDirectory()) {
            logForDebugging(
              `[Nexus in Chrome] Detected browser: ${config.name}`,
            )
            return browserId
          }
        } catch (e) {
...
export async function openInChrome(url: string): Promise<boolean> {
  ...
  if (!browser) {
    logForDebugging('[Nexus in Chrome] No compatible browser found')
    return false
  }
...
function getSocketName(): string {
  // NOTE: This must match the one used in the Nexus in Chrome MCP
