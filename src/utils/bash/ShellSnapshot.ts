...
/**
 * Generates Nexus Code specific snapshot content
...
RIPGREP_FUNC_END
  ...
  // consistent fast behavior in Nexus's shell.
...
async function getSnapshotScript(
  ...
  // Generate the user content and Nexus Code content
 ...
 *
 * This function is a critical part of Nexus CLI's shell integration strategy. It:
 *
 ...
 *
 * This approach allows Nexus CLI to execute commands as if they were run in the user's
...
export const createAndSaveSnapshot = async (
  ...
  return new Promise(async resolve => {
    try {
      ...
      if (!configFileExists) {
        logForDebugging(
          `Shell config file not found: ${configFile}, creating snapshot with Nexus Code defaults only`,
        )
      }
      ...
      execFile(
        ...
        async (error, stdout, stderr) => {
          if (error) {
            ...
            logForDebugging(`  - Nexus home: ${getClaudeConfigHomeDir()}`)
