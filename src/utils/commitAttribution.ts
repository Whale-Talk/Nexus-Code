...
/**
 * Attribution state for tracking Nexus's contributions to files.
 */
...
/**
 * Summary of Nexus's contribution for a commit.
 */
...
function computeFileModificationState(
  ...
  try {
    // Calculate Nexus's character contribution
...
/**
 * Track a file modification by Nexus.
...
/**
 * Track a file creation by Nexus (e.g., via bash command).
 * Used when Nexus creates a new file through a non-tracked mechanism.
 */
...
/**
 * Track a file deletion by Nexus (e.g., via bash rm command).
 * Used when Nexus deletes a file through a non-tracked mechanism.
 */
  ...
  const fileResults = await Promise.all(
    stagedFiles.map(async file => {
      ...
      if (deleted) {
        // File was deleted
        if (fileState) {
          // Nexus deleted this file (tracked deletion)
