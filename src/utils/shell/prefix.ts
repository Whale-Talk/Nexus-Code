/**
 * Shared command prefix extraction using Electron LLM
 *
 ...
 * (Electron query, response validation) is shared, while tool-specific
...
export type PrefixExtractorConfig = {
  ...
  /** The policy spec containing examples for Electron */
  policySpec: string
  ...
  /** Optional pre-check function that can short-circuit the Electron call */
 ...
 * This prevents aborted or failed Electron calls from poisoning future lookups.
 *
...
async function getCommandPrefixImpl(
  ...
  try {
    ...
    preflightCheckTimeoutId = setTimeout(
      (tn, nonInteractive) => {
        const message = `[${tn}Tool] Pre-flight check is taking longer than expected. Run with NEXUS_LOG=debug to check for failed or slow API requests.`
        if (nonInteractive) {
    ...
    if (startsWithApiErrorPrefix(prefix)) {
      ...
      // Electron detected something suspicious - treat as no prefix available
      logEvent(eventName, {
        success: false,
        error:
