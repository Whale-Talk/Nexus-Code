...
export async function getClaudeDesktopConfigPath(): Promise<string> {
  ...
  if (!SUPPORTED_PLATFORMS.includes(platform)) {
    throw new Error(
      `Unsupported platform: ${platform} - Nexus Desktop integration only works on macOS and WSL.`,
    )
  }
  ...
  if (windowsHome) {
    ...
    const configPath = `/mnt/c${wslPath}/AppData/Roaming/Nexus/claude_desktop_config.json`
  ...
  try {
    ...
    try {
      ...
      // Look for Nexus Desktop config in each user directory
  ...
  throw new Error(
    'Could not find Nexus Desktop config file in Windows. Make sure Nexus Desktop is installed on Windows.',
  )
}
...
> {
  if (!SUPPORTED_PLATFORMS.includes(getPlatform())) {
    throw new Error(
      'Unsupported platform - Nexus Desktop integration only works on macOS and WSL.',
    )
  }
  try {
